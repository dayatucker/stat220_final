...
server <- function(input, output, session) {
           ...
           "info_tab" = {
             tabPanel("Info",
                      mainPanel(
                        ...
                        p("🎧 Start with a Random Song Spotlight above, or jump straight into the trends that shaped streaming between 2018 and 2024 by clicking on the menu button in the top left corner.")
                      ))
           },
           "artists" = {
  ...
  output$song_spotlight <- renderUI({
    ...
    fire <- get_popularity_fire(spotlight_song$popularity)
  ...
  filtered_artists <- reactive({
    get_filtered_artists(combined_artists_tracks, input$selected_year)
  })
  
  filtered_albums_year <- reactive({
    get_filtered_albums_year(combined_albums_tracks, input$selected_year)
  })
