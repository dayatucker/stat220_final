...
```{r}
combined_album_all %>% 
  filter(charted_year == 2023) %>% 
  select(album_id, album_name, album_popularity, album_total_tracks) %>% 
  distinct() %>% 
  ggplot() + 
  geom_col(aes(x = factor(album_name, levels = unique(album_name)), y = album_popularity)) + 
  theme_minimal() +
