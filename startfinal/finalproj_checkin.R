...
# Pre-fetch all artists' top tracks and combine into one big table
...
server <- function(input, output, session) {
  ...
  # Reactive filtered tracks for table
  top_tracks_table_data <- reactive({
    req(input$selected_artist_table)
    if (input$selected_artist_table == "all") {
      all_tracks |>
        arrange(desc(popularity)) |>
        slice_head(n = 10)
    } else {
      filter(all_tracks, artist_id == input$selected_artist_table)
    }
  })
  ...
  # Reactive filtered tracks for plot with explicit filter
  top_tracks_plot_data <- reactive({
    req(input$selected_artist_plot)
    tracks <- if (input$selected_artist_plot == "all") {
      all_tracks
    } else {
      filter(all_tracks, artist_id == input$selected_artist_plot)
    }
